...
from model.backbone.apes_cls_backbone import APESClsBackbone, simple_mlp, cp_mlp
from model.head.apes_cls_head import MLP_Head, NewOut
...
class Model(nn.Module):
    def __init__(self, template_params):
        ...
        self.backbone = APESClsBackbone('global')
        self.head = NewOut(self.cp_num)
        self.cp_mlp = cp_mlp()

    def forward(self, cp):
        cp = rearrange(cp, 'B N C -> B (N C)') # (B, 366)
        output = self.cp_mlp(cp)
        output = rearrange(output, 'B (N C) -> B N C', C = 3)
...
def training(**kwargs):
    ...
    template_noise = torch.rand_like(template_params)
    ...
    #     vertices = model() # model is changed!!!!!
    ...
    for i in loop:
        vertices = model(template_noise) # MLP:(B, N), review (B, -1, 3)
        patches = vertices[:,face_idx] # (B, face_num, cp_num, 3)
        curves = vertices[:,curve_idx] # (B, curve_num, cp_num, 3)
